...
weight_init = tf.keras.initializers.variance_scaling()
weight_regularizer = tf.keras.regularizers.l2(0.00005)
...
def resblock(x_init, channels, is_training=True, use_bias=True, downsample=False, scope='resblock') :
    with tf.compat.v1.variable_scope(scope) :
        ...
        x = batch_norm(x_init, is_training, NAME='batch_norm_0')
        x = relu(x)
        ...
        x = batch_norm(x, is_training, NAME='batch_norm_1')
        x = relu(x)
...
def bottle_resblock(x_init, channels, is_training=True, use_bias=True, downsample=False, scope='bottle_resblock') :
    with tf.compat.v1.variable_scope(scope) :
        x = batch_norm(x_init, is_training, NAME='batch_norm_1x1_front')
        shortcut = relu(x)

        x = conv(shortcut, channels, kernel=1, stride=1, use_bias=use_bias, scope='conv_1x1_front')
        x = batch_norm(x, is_training, NAME='batch_norm_3x3')
        x = relu(x)
        ...
        x = batch_norm(x, is_training, NAME='batch_norm_1x1_back')
        x = relu(x)
...
def batch_norm(x, is_training=True, NAME='batch_norm'):
    return BatchNormalization(name=NAME,momentum = 0.99, epsilon=1e-05,center=True, scale=True)(x)
