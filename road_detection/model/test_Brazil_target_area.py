...
from decoder_nooutput import DecodeAndVis 
...
parser.add_argument('-dataset', action='store', dest='dataset', type=str,
                    help='dataset ', required =False, default=None)

parser.add_argument('-mesh_polygon', action='store', dest='mesh_polygon', type=str,
                    help='mesh polygon ', required =False, default=None)
...
osmdataset = args.dataset
mesh_path = args.mesh_polygon

gdf_mesh =gpd.read_file(mesh_path)
...
with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(gpu_options=gpu_options)) as sess:
	...
	if args.spacenet == "":
		...
		for i in range(len(gdf_mesh)):
			num = gdf_mesh.loc[i,"ID"]
			indrange_test.append(int(num))
		...
		if args.mode == "train":
			...
			for tile_id in tiles:
				...
				gdf_mesh1 = gdf_mesh[gdf_mesh['ID'] == tile_id]

				x_max = gdf_mesh1.loc[:,"lon_ed"].iloc[-1]
				x_min = gdf_mesh1.loc[:,"lon_st"].iloc[-1]
				y_max = gdf_mesh1.loc[:,"lat_st"].iloc[-1]
				y_min = gdf_mesh1.loc[:,"lat_ed"].iloc[-1]

				xres = (x_max - x_min) / 2048
				yres = (y_max - y_min) / 2048

				transform = Affine.translation(x_min - xres / 2, y_min - yres / 2) * Affine.scale(xres, yres)
				...
				outputfile = "outputs/" + str(tile_id) + "_output_keypoints.tif"
                
				with rasterio.open(
                        outputfile,
                        'w',
                        driver="GTiff",
                        height=output_keypoints_img.shape[0],
                        width=output_keypoints_img.shape[1],
                        count=1,
                        dtype=output_keypoints_img.dtype,
                        crs=CRS.from_epsg(4326),
                        transform=transform,
				) as new_dataset:
				        new_dataset.write(output_keypoints_img, 1)

				input_sat_img = ((input_sat[0,:,:,:]+0.5) * 255.0).reshape((2048,2048,3)).astype(np.uint8)
				input_sat_img_1 = input_sat_img[:,:,0]
				input_sat_img_2 = input_sat_img[:,:,1]
				input_sat_img_3 = input_sat_img[:,:,2]


				outputfile2 = "outputs/" + str(tile_id) + "_sat.tif"

				with rasterio.open(
                        outputfile2,
                        'w',
                        driver="GTiff",
                        height=input_sat_img_1.shape[0],
                        width=input_sat_img_1.shape[1],
                        count=3,
                        dtype=input_sat_img_1.dtype,
                        crs=CRS.from_epsg(4326),
                        transform=transform,
				) as new_dataset:
				        new_dataset.write(input_sat_img_1, 1)
				        new_dataset.write(input_sat_img_2, 2)				        
				        new_dataset.write(input_sat_img_3, 3)
