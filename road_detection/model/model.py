...
class Sat2GraphModel():
	def __init__(self, sess, image_size=352, image_ch = 3, downsample_level = 1, batchsize = 8, resnet_step=8, channel=12, mode = "train", joint_with_seg=True):
		...
		self.input_sat = tf.compat.v1.placeholder(tf.float32, shape = [self.batchsize, self.image_size, self.image_size, self.image_ch], name="input")

		self.input_seg_gt = tf.compat.v1.placeholder(tf.float32, shape = [self.batchsize, self.image_size, self.image_size, 1])
		self.input_seg_gt_target = tf.concat([self.input_seg_gt+0.5, 0.5 - self.input_seg_gt], axis=3)

		self.target_prob = tf.compat.v1.placeholder(tf.float32, shape = [self.batchsize, self.image_size, self.image_size, 2 * (MAX_DEGREE + 1)])
		self.target_vector = tf.compat.v1.placeholder(tf.float32, shape = [self.batchsize, self.image_size, self.image_size, 2 * (MAX_DEGREE)])
		...
		if self.train_seg:
			...
			num_unet = len(tf.compat.v1.trainable_variables())
			print("Weights", num_unet)

			self.output = tf.nn.softmax(self.linear_output)
			self.loss = tf.reduce_mean(tf.compat.v1.losses.softmax_cross_entropy(self.input_seg_gt_target, self.linear_output))
			...
			self.l2loss_grad = tf.gradients(self.loss, tf.compat.v1.trainable_variables())
		...
		self.l2_grad = tf.compat.v1.placeholder(tf.float32)

		self.summary_loss.append(tf.compat.v1.summary.scalar('loss/test', self.test_loss))
		self.summary_loss.append(tf.compat.v1.summary.scalar('loss/train', self.train_loss))
		self.summary_loss.append(tf.compat.v1.summary.scalar('grad/l2', self.l2_grad))
	...
	def class_resnet_blocks(self, x, ch, name, resnet_step=0):
		if resnet_step > 0:
			...
			x = batch_norm_resnet(x, NAME = name+"_decode_0_batch_norm") # roll back
	...
	def DownSampleSimilarityLoss(self, output, target):
		...
		y = tf.nn.avg_pool2d(input=output, ksize=[1,3,3,1], strides=[1,2,2,1],padding='SAME')
		y = tf.nn.avg_pool2d(input=y, ksize=[1,3,3,1], strides=[1,2,2,1],padding='SAME')
		y = tf.nn.avg_pool2d(input=y, ksize=[1,3,3,1], strides=[1,2,2,1],padding='SAME')
		y = tf.nn.avg_pool2d(input=y, ksize=[1,3,3,1], strides=[1,2,2,1],padding='SAME')
		
		x = tf.nn.avg_pool2d(input=target, ksize=[1,3,3,1], strides=[1,2,2,1],padding='SAME')
		x = tf.nn.avg_pool2d(input=x, ksize=[1,3,3,1], strides=[1,2,2,1],padding='SAME')
		x = tf.nn.avg_pool2d(input=x, ksize=[1,3,3,1], strides=[1,2,2,1],padding='SAME')
		x = tf.nn.avg_pool2d(input=x, ksize=[1,3,3,1], strides=[1,2,2,1],padding='SAME')
	...
	def SupervisedLoss(self, imagegraph_output, imagegraph_target_prob, imagegraph_target_vector):
		...
		keypoint_prob_loss = tf.reduce_mean(tf.compat.v1.losses.softmax_cross_entropy(keypoint_prob_target, keypoint_prob_output))
		...
		for i in range(MAX_DEGREE):
			...
			# only at key points! 
			direction_prob_loss += tf.reduce_mean(tf.multiply((soft_mask2), tf.compat.v1.losses.softmax_cross_entropy(prob_target, prob_output, reduction=tf.compat.v1.losses.Reduction.NONE)))
		...
		if self.joint_with_seg:
			...
			seg_loss = tf.reduce_mean(tf.compat.v1.losses.softmax_cross_entropy(self.input_seg_gt_target, tf.concat([imagegraph_outputs[2+MAX_DEGREE*4], imagegraph_outputs[2+MAX_DEGREE*4+1]], axis=3)))
	...
	def BuildDeepLayerAggregationNetWithResnet(self, net_input, input_ch = 3, output_ch = 26, ch = 24):
		...
		def resnet_blocks(x, ch, name, resnet_step=0):
			if resnet_step > 0:
				...
				x = batch_norm_resnet(x, is_training=self.is_training, NAME = name+"_decode_0_batch_norm") # mark
