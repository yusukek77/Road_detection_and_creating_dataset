...
class OSMLoader:
    def __init__(self, region, noUnderground = False, osmfile=None, includeServiceRoad = False):
        ...
        roadForMotorDict = {'motorway','trunk','primary','secondary','tertiary','unclassified','residential','motorway_link','trunk_link','primary_link','secondary_link','tertiary_link','service','track','track_grade1','track_grade2','track_grade3','track_grade4','track_grade5','path'}

        roadForMotorBlackList = {'unknown','None','living_street','pedestrian','bus_guideway','escape','raceway','road','busway','footway','bridleway','steps','corridor','cycleway','proposed','construction','bus_stop','crossing','elevator','emergency_bay','emergency_access_point','give_way','milestone','mini_roundabout','motorway_junction','passing_place','platform','rest_area','services','speed_camera','stop','street_lamp','toll_gantry','traffic_mirror','traffic_signals','trailhead','turning_circle','turning_loop','User Defined'}
        ...
        for away in ways:
            ...
            for atag in away.findall('tag'):
                ...
                if atag.get('k') == 'lanes':
                    try:
                        lanes = float(atag.get('v').split(';')[0])#;の間で分割する
                    except ValueError:
                        lanes = -1 
            ...
            if highway not in roadForMotorBlackList and (includeServiceRoad == True or parking == False): 
